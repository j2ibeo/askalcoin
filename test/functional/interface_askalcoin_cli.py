...
# The block reward of coinbaseoutput.nValue (50) STC/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 STC/block.
...
class TestAskalcoinCli(AskalcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `askalcoin-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and askalcoin-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and askalcoin-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (STC/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (STC/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 STC) to the default wallet.
            ...
            # Test askalcoin-cli -generate with -rpcwallet in multiwallet mode.
