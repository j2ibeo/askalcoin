...
def assert_fee_amount(fee, tx_size, feerate_STC_kvB):
    """Assert the fee is in range."""
    feerate_STC_vB = feerate_STC_kvB / 1000
    target_fee = satoshi_round(tx_size * feerate_STC_vB)
    if fee < target_fee:
        raise AssertionError("Fee of %s STC too low! (Should be %s STC)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * feerate_STC_vB:
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting STC values"""
...
def wait_until_helper(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None, timeout_factor=1.0):
    ...
    from `AskalcoinTestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "askalcoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "askalcoin.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "askalcoin.conf")):
        with open(os.path.join(datadir, "askalcoin.conf"), 'r', encoding='utf8') as f:
            for line in f:
