...
// For details see https://github.com/askalcoin/askalcoin/pull/22348.
...
const char * const ASKALCOIN_CONF_FILENAME = "askalcoin.conf";
const char * const ASKALCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/askalcoin/askalcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //askalcoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", ASKALCOIN_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "askalcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Askalcoin
    // macOS: ~/Library/Application Support/Askalcoin
    // Unix-like: ~/.askalcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Askalcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Askalcoin";
#else
    // Unix-like
    return pathRet / ".askalcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", ASKALCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Askalcoin Core copyright is not removed by accident
    if (copyright_devs.find("Askalcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Askalcoin Core developers";
    }
