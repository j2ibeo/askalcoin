...
#include <qt/askalcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Askalcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new AskalcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new AskalcoinAddressCheckValidator(parent));
}
...
bool parseAskalcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no askalcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("askalcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!AskalcoinUnits::parse(AskalcoinUnits::STC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseAskalcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseAskalcoinURI(uriInstance, out);
}

QString formatAskalcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("askalcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(AskalcoinUnits::format(AskalcoinUnits::STC, info.amount, false, AskalcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openAskalcoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", ASKALCOIN_CONF_FILENAME));
    ...
    /* Open askalcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Askalcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Askalcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Askalcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Askalcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "askalcoin.desktop";
    return GetAutostartDir() / strprintf("askalcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a askalcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Askalcoin\n";
        else
            optionFile << strprintf("Name=Askalcoin (%s)\n", chain);
