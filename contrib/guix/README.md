# Bootstrappable Askalcoin Core Builds

This directory contains the files necessary to perform bootstrappable Askalcoin
Core builds.
...
To build Askalcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Askalcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/askalcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/askalcoin-core/guix.sigs).
