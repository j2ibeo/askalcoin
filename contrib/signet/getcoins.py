...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing askalcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='askalcoin-cli', help='askalcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Askalcoin address to which the faucet should send')
...
parser.add_argument('askalcoin_cli_args', nargs='*', help='Arguments to pass on to askalcoin-cli (default: -signet)')

args = parser.parse_args()

if args.askalcoin_cli_args == []:
    args.askalcoin_cli_args = ['-signet']


def askalcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.askalcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = askalcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = askalcoin_cli(['getnewaddress', 'faucet', 'bech32'])
