...
The Askalcoin Core daemon can be configured to act as a trusted "border
router", implementing the askalcoin wire protocol and relay, making
...
The ZeroMQ feature in Askalcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building askalcoind:
...
For instance:
    ...
    $ askalcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubhashblock="tcp://[::1]:29999" \
               -zmqpubrawtx=ipc:///tmp/askalcoind.tx.raw \
...
These options can also be provided in askalcoin.conf.
...
From the perspective of askalcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into askalcoind directly. Furthermore, no information is
...
using. Askalcoind appends an up-counting sequence number to each
