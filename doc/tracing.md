# User-space, Statically Defined Tracing (USDT) for Askalcoin Core

Askalcoin Core includes statically defined tracepoints to allow for more
...
## Adding tracepoints to Askalcoin Core
...
Multiple tools can list the available tracepoints in a `askalcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Askalcoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/askalcoind
stap net        outbound_message 0x0000000000107c05 /src/askalcoind
stap validation block_connected  0x00000000002fb10c /src/askalcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Askalcoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/askalcoind | grep NT_STAPSDT -A 4 -B 2
