*After branching off for a major version release of Askalcoin Core, use this
...
https://github.com/askalcoin-core/askalcoin-devwiki/wiki/
...
Askalcoin Core version *version* is now available from:

  <https://askalcoincore.org/bin/askalcoin-core-*version*/>
...
using the Linux kernel, macOS 10.15+, and Windows 7 and newer.  Askalcoin
...
- A askalcoind node will no longer rumour addresses to inbound peers by default.
...
* On startup, the list of banned hosts and networks (via `setban` RPC) in
  `banlist.dat` is ignored and only `banlist.json` is considered. Askalcoin Core
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from Askalcoin Core in 22.0. (#22544)
