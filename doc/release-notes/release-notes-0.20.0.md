...
Askalcoin Core version 0.20.0 is now available from:

  <https://askalcoincore.org/bin/askalcoin-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `ASKALCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Askalcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Askalcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Askalcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Askalcoin Core. Askalcoin Core logs debug messages
...
- OpenSSL is no longer used by Askalcoin Core.  (#17265)

- BIP70 support has been fully removed from Askalcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/askalcoin/askalcoin/issues/16599),
  [PR #16702](https://github.com/askalcoin/askalcoin/pull/16702), and the
  `askalcoind help` for more information.  This option is experimental and
...
- All custom settings configured when Askalcoin Core starts are now
...
- The "Start Askalcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://askalcoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Askalcoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Askalcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/askalcoin-core/HWI) for
  signing.  Future versions of Askalcoin Core should support a GUI option
...
- Askalcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.askalcoincore.org.  (#17596)
...
- The `askalcoin-cli` utility used with the `-getinfo` parameter now
...
- The `askalcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `askalcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_askalcoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBASKALCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_askalcoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBASKALCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for askalcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate askalcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest askalcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_askalcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (askalcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone askalcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Askalcoin Core files (hebasto)
...
- #17119 Fix broken askalcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from askalcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.askalcoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to askalcoin-wallet (hel-o)
...
- #18341 Replace remaining literal STC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On askalcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from askalcoin-cli -getinfo (malevolent)
