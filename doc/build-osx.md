...
This guide describes how to build askalcoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Askalcoin Core from source.
...
### 4. Clone Askalcoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Askalcoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `askalcoind` or  `askalcoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/askalcoin/askalcoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Askalcoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/askalcoin/askalcoin/issues/7714).
...
You can deploy a `.dmg` containing the Askalcoin Core application using `make deploy`.
...
Run the following in your terminal to compile Askalcoin Core:
...
## Running Askalcoin Core

Askalcoin Core should now be available at `./src/askalcoind`.
If you compiled support for the GUI, it should be available at `./src/qt/askalcoin-qt`.

The first time you run `askalcoind` or `askalcoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Askalcoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Askalcoin"

touch "/Users/${USER}/Library/Application Support/Askalcoin/askalcoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Askalcoin/askalcoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Askalcoin/debug.log
```

## Other commands:

```shell
./src/askalcoind -daemon      # Starts the askalcoin daemon.
./src/askalcoin-cli --help    # Outputs a list of command-line options.
./src/askalcoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/askalcoin-qt -server # Starts the askalcoin-qt server mode, allows askalcoin-cli control
```
